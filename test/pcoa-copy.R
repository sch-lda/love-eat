...
rm(list = ls())
# 自动安装并加载必要的包
required_packages <- c("readxl", "vegan", "ggplot2", "dplyr", "openxlsx")
new_packages <- required_packages[!(required_packages %in% installed.packages()[,"Package"])]
if(length(new_packages)) install.packages(new_packages, repos = "https://cloud.r-project.org")


library(readxl)   # 读取Excel
library(openxlsx) # 读取xlsx（提供read.xlsx函数）
library(vegan)    # 计算距离和PCoA
library(ggplot2)  # 绘图
library(dplyr)    # 数据处理

# 2. 设置文件路径 (请将文件放在工作目录下，或写完整路径)
meta_data <- read.xlsx("D:/R/love-eat/test/h1yuanshiturangxijunchouping(1).xlsx",sheet = 2)   # 包含 Site 和 Group 信息
otu_data  <- read.xlsx("D:/R/love-eat/test/h1yuanshiturangxijunchouping(1).xlsx",sheet = 1)   # 包含 Taxonomy 和 丰度信息
...
cat("分析完成！图片已保存为 PCoA_Result2.png\n")