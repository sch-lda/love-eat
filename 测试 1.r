...
meta_file <- "./h4.xlsx"   # 包含 Site 和 Group 信息
otu_file  <- "./h3.xlsx"   # 包含 Taxonomy 和 丰度信息
...
p <- ggplot(final_df, aes(x = PCoA1, y = PCoA2, color = Group, label = Sample)) +
  ...
  stat_ellipse(aes(color = Group), 
               type = "norm",  # 正态分布假设下的椭圆
               level = 0.95,   # 95%置信区间
               linetype = "solid",
               linewidth = 0.8) +  # 使用linewidth替代size（避免警告）
...
cat("分析完成！图片已保存为 \"D:\\R\\love-eat\\PCoA_Result.png\"\n")